...
void Renderer::LoadCubeMap()
{
    HRESULT hr = CreateDDSTextureFromFile(
        mDevice.Get(),
        L"Textures \ \ output_skybox.dds ",
        nullptr,
        SkyboxTextureView.GetAddressOf());

    if (FAILED(hr))
    {
        OutputDebugString(L"Failed to load Cube Map DDS! \ n");
    }
    else
    {
        OutputDebugString(L"Successfully loaded Cube Map DDS! \ n");
    }

    //std::string skyboxVSCode = ReadHLSLFile(" SkyBoxVertexShader.hlsl ");
    //std::string skyboxPSCode = ReadHLSLFile(" SkyBoxPixelShader.hlsl ");

    ComPtr<ID3DBlob> errorBlob;
    const std::string SkyboxpixelShaderCode = R"(
    ...
    TextureCube texObject : register(t1);
    SamplerState objSampleState : register(s1);

    float4 main(PS_INPUT input) : SV_TARGET
    {
        float3 lookup = normalize(input.direction);
        return texObject.Sample(objSampleState, lookup);
    }
    )";

    D3DCompile(SkyboxpixelShaderCode.c_str(), SkyboxpixelShaderCode.length(), nullptr, nullptr, nullptr, "main", "ps_5_0",
        D3DCOMPILE_ENABLE_STRICTNESS, 0, &mSkyboxVertexShaderBlob, &errorBlob);

    if (errorBlob.Get() != nullptr && errorBlob->GetBufferPointer() != nullptr)
        OutputDebugString((LPCWSTR)errorBlob->GetBufferPointer());
    if (FAILED(mDevice->CreatePixelShader(mSkyboxVertexShaderBlob->GetBufferPointer(), mSkyboxVertexShaderBlob->GetBufferSize(), nullptr, mPixelShader.GetAddressOf())))
    {
        OutputDebugString(L"Failed to craete Pixel Shader!\n");
        abort();
    }
    ...
    ComPtr<ID3DBlob> errorBlob1;
    const std::string SkyboxvertexShaderCode = R"(
    ...
    D3DCompile(SkyboxvertexShaderCode.c_str(), SkyboxvertexShaderCode.length(), nullptr, nullptr, nullptr, "main", "vs_5_0",
        D3DCOMPILE_ENABLE_STRICTNESS, 0, &mSkyboxVertexShaderBlob, &errorBlob1);

    if (errorBlob1.Get() != nullptr && errorBlob1->GetBufferPointer() != nullptr)
        OutputDebugString((LPCWSTR)errorBlob1->GetBufferPointer());

    if (FAILED(mDevice->CreateVertexShader(mSkyboxVertexShaderBlob->GetBufferPointer(), mSkyboxVertexShaderBlob->GetBufferSize(), nullptr, mSkyboxVertexShader.GetAddressOf())))
    {
        OutputDebugString(L"Failed to create Vertex Shader!\n");
        abort();
    }
    else {
        OutputDebugString(L"Successful to create Vertex Shader!\n");

    }

    ComPtr <ID3DBlob > skyboxErrorBlob = nullptr;
    hr = D3DCompile(
        SkyboxvertexShaderCode.c_str(), SkyboxvertexShaderCode.length(),
        nullptr, nullptr, nullptr,
        "main", "vs_5_0", D3DCOMPILE_ENABLE_STRICTNESS, 0,
        &mSkyboxVertexShaderBlob, &skyboxErrorBlob);

    mDevice->CreateVertexShader(
        mSkyboxVertexShaderBlob -> GetBufferPointer(),
        mSkyboxVertexShaderBlob -> GetBufferSize(),
        nullptr, &mSkyboxVertexShader);

    hr = D3DCompile(
        SkyboxpixelShaderCode.c_str(), SkyboxpixelShaderCode.length(),
        nullptr, nullptr, nullptr,
        "main", "ps_5_0", D3DCOMPILE_ENABLE_STRICTNESS, 0,
        &mSkyboxPixelShaderBlob, &skyboxErrorBlob);
    mDevice -> CreatePixelShader(
        mSkyboxPixelShaderBlob -> GetBufferPointer(),
        mSkyboxPixelShaderBlob -> GetBufferSize(),
        nullptr, &mSkyboxPixelShader);
...
D3D11_INPUT_PER_VERTEX_DATA , 0 }
    ...
    D3D11_BUFFER_DESC skyboxCBD = {};
    skyboxCBD.Usage = D3D11_USAGE_DEFAULT;
    skyboxCBD.ByteWidth = sizeof(SkyboxConstantBuffer);
    skyboxCBD.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    skyboxCBD.CPUAccessFlags = 0;
    mDevice -> CreateBuffer(&skyboxCBD, nullptr, &mSkyboxConstantBuffer);
}

void Renderer::CreateSkyboxGeometry()
{
    SkyboxVertexData skyboxVertices[] = {
        // Front Face
        { { -1.0f, -1.0f, -1.0f} },
        { { -1.0f, 1.0f, -1.0f}},
        { { 1.0f, 1.0f, -1.0f} },
   ...
   // mSkyboxIndexCount = sizeof(skyboxIndices) / sizeof(skyboxIndices[0]);

    D3D11_BUFFER_DESC vbDesc1 = {};
    vbDesc1.Usage = D3D11_USAGE_DEFAULT;
    vbDesc1.ByteWidth = sizeof(skyboxVertices);
    vbDesc1.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    D3D11_SUBRESOURCE_DATA vbData1 = {};
    vbData1.pSysMem = skyboxVertices;
    HRESULT hr = mDevice -> CreateBuffer(&vbDesc1, &vbData1,
        SkyboxVertexBuffer.GetAddressOf());
    if (FAILED(hr))
    {
        OutputDebugString(L"Failed to create skybox vertex buffer \ n");
        return;
    }
    D3D11_BUFFER_DESC ibDesc1 = {};
    ibDesc1.Usage = D3D11_USAGE_DEFAULT;
    ibDesc1.ByteWidth = sizeof(skyboxIndices);
    ibDesc1.BindFlags = D3D11_BIND_INDEX_BUFFER;
    D3D11_SUBRESOURCE_DATA ibData1 = {};
    ibData1.pSysMem = skyboxIndices;
    hr = mDevice -> CreateBuffer(&ibDesc1, &ibData1,
        SkyboxIndexBuffer.GetAddressOf());
    if (FAILED(hr))
    {
        OutputDebugString(L"Failed to create skybox index buffer \ n");
        return;
    }
    OutputDebugString(L"Successfully created skybox geometry \ n");
}
